...
async function main() {
  ...
  const client = new WillowClient({
    ...
    did: 'did:willow:local:app_owner',
  ...
  try {
    ...
    const userData = await client.get(appId, datasetId, 'user123');
    ...
    const queryResult = await client.query(appId, datasetId, {
      filters: { active: true },
      limit: 10
    });
    ...
    const unverifiedData = await client.getUnverified(appId, datasetId, 'user123');
    console.log('Unverified data retrieved:', unverifiedData);

    // 4. Collection helper also supports both modes
    const collection = client.collection(appId, datasetId);
