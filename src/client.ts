...
export class WillowClient {
   ...
   * Store data
   */
  async store(appId: string, datasetId: string, key: string, value: any): Promise<void> {
   ...
   */
  async get(appId: string, datasetId: string, key: string): Promise<DataRecord> {
   ...
   */
  async getUnverified(appId: string, datasetId: string, key: string): Promise<DataRecord> {
   ...
   * Update data
   */
  async update(appId: string, datasetId: string, key: string, value: any): Promise<void> {
   ...
   * Get proof
   */
  async getProof(appId: string, datasetId: string, key: string): Promise<string> {
  ...
  async getRootHash(): Promise<string> {
    const response = await fetch(`${this.config.apiUrl}/state/root-hash/verified`);
    if (!response.ok) {
   ...
   */
  async query(appId: string, datasetId: string, query: QueryRequest): Promise<QueryResponse> {
   ...
   */
  async queryUnverified(appId: string, datasetId: string, query: QueryRequest): Promise<QueryResponse> {
  ...
  collection(appId: string, datasetId: string) {
    return {
      store: (key: string, value: any) => this.store(appId, datasetId, key, value),
      get: (key: string) => this.get(appId, datasetId, key),
      getUnverified: (key: string) => this.data.getDataUnverified(appId, datasetId, key),
      update: (key: string, value: any) => this.update(appId, datasetId, key, value),
      ...
      getMultiple: (keys: string[]) => this.data.getMultiple(appId, datasetId, keys),
      getMultipleUnverified: (keys: string[]) => this.data.getMultipleUnverified(appId, datasetId, keys),
      query: (query: any) => this.data.query(appId, datasetId, query),
      queryUnverified: (query: any) => this.data.queryUnverified(appId, datasetId, query),
    };
  }
}